...
[![Docker Image](https://img.shields.io/badge/Docker-ghcr.io%2Ft4wroot%2Frex--node-blue?style=flat-square&logo=docker)](https://github.com/T4wroot/rex/pkgs/container/rex-node)
...
Run this command on Ubuntu / Debian servers to add the repository and install `rex-node`:

```bash
curl -fsSL https://raw.githubusercontent.com/T4wroot/rex/master/rex-node/install.sh | bash -s -- --token YOUR_TOKEN
```
*(Or download and install the `.deb` package directly:)*
```bash
...
Pull and run the official GitHub Container Registry image:
...
docker run -d \
  ...
  -p 7443:7443 \
  -v /etc/rex:/etc/rex \
  ghcr.io/t4wroot/rex-node:latest
```
